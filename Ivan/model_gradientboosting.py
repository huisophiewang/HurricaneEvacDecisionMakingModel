...
def modelfit(alg, dtrain, predictors, target, performCV=True, printFeatureImportance=False, cv_folds=10):
...
def tune_paramenters(fp):
    df = pd.read_csv(fp)
    target = 'evac'
    predictors = [x for x in df.columns if x not in [target]]
#     cls = GradientBoostingClassifier()
#     plot_feature_rank(cls, df, predictors, target)

    
#     gbm0 = GradientBoostingClassifier(random_state=0)
#     modelfit(gbm0, df, predictors, target)
...
#     #{'min_samples_split': 100, 'min_samples_leaf': 60}
    ...
    final_model = GradientBoostingClassifier(learning_rate=0.1, 
                                               ...
                                               subsample=0.8, 
                                               random_state=0)
    #final_model.fit(df[predictors], df[target])
    modelfit(final_model, df, predictors, target)
